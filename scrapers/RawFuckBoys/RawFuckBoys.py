...
BASE_URL = "https://rawfuckboys.com"
SITEMAP_URL = "https://rawfuckboys.com/sitemap.xml"
    ...
    for url in urls:
        try:
            ...
            if page_resp.status_code != 200:
                continue
            match = re.search(r'content/(rfb\d+)/', page_resp.text)
            if match:
...
def scrape_scene(url):
    """Scrape a scene from a RawFuckBoys URL."""
    soup = fetch_page(url)
    if not soup:
        return {}
    ...
    if og_image and og_image.get("content"):
        code_match = re.search(r'content/(rfb\d+)/', og_image["content"])
        if code_match:
            result["code"] = code_match.group(1)

    # Studio
    result["studio"] = {"name": "Raw Fuck Boys"}
...
def scrape_performer(url):
    """Scrape a performer from a RawFuckBoys model URL."""
    soup = fetch_page(url)
    if not soup:
        return {}
...
def extract_scene_code(text):
    """Extract a RawFuckBoys scene code (e.g. rfb0018) from a filename or string."""
    m = re.search(r'(?i)(?:^|[_\-\s])(rfb\d{3,5})(?:$|[_\-\s.])', text)
    if m:
...
def scene_by_fragment():
    ...
    if scene_url and "rawfuckboys.com/videos/" in scene_url:
    ...
    if not code:
        log.info(f"No RawFuckBoys scene code found in '{source}'")
        print(json.dumps({}))
        return
