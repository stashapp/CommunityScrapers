...
def scrape_url(url, scrape_type):
    ...
    if domain not in STUDIOS:
        log.error(f'Domain {domain} not supported, URL: {url}')
        return None
    path = parsed.path.split('/')
    base_url = f'https://{STUDIOS[domain]["api"]}'
    if scrape_type == 'scene':
        try:
            match = re.match(r'(?P<id>\d+)(\-(?P<name>.+))?$', path[path.index('videos') + 1])
            scraped = scrape_videos(base_url, domain=domain, id=match.group('id'), name=match.group('name'))
        except ValueError:
            log.error(f'Scene scraping not supported for {url}')
            return None
    elif scrape_type == 'gallery':
        try:
            match = re.match(r'(?P<id>\d+)(\-(?P<name>.+))?$', path[path.index('photosets') + 1])
            scraped = scrape_photosets(base_url, domain=domain, id=match.group('id'), name=match.group('name'))
        except ValueError:
...
def fetch(base_url, fetch_type, id):
    url = f'{base_url}/api/{fetch_type}/{id}'
...
def fetch_image(response):
    for image_type in ['poster_src', 'cover_photo']:
        if image_type in response:
            response['Image'] = response[image_type]
            try:
                response = requests.get(response['Image'], headers={'User-Agent': USER_AGENT}, timeout=(3, 6))
...
def map_media(response, type, domain):
    info = {
        'Title': response['title'],
        'Details': BeautifulSoup(response['description'], 'html.parser').get_text(),
        'URLs': [f'https://{domain}/{type}/{response["id"]}'],
        ...
        'Studio': {
            'Name': STUDIOS[domain]['name'] if domain in STUDIOS else '',
            'URL': f'https://{domain}',
        },
        'Code': str(response['id']),
    }
    if type == 'videos':
        info['Image'] = fetch_image(response)
    return info


def scrape_videos(base_url, domain, id, name):
    log.info(f'Scraping scene \'{name}\' (ID {id})')
    data = fetch(base_url, 'videos', id)
    return map_media(data, 'videos', domain) if data is not None else None


def scrape_photosets(base_url, domain, id, name):
    log.info(f'Scraping gallery \'{name}\' (ID {id})')
    data = fetch(base_url, 'photo-collections', id)
    return map_media(data, 'photosets', domain) if data is not None else None
