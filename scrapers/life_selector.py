...
class LifeSelectorScraper(base_python_scraper.BasePythonScraper):
    ...
    def _get_scene_by_fragment(self, fragment: dict) -> dict:
        '''
        Get scene properties by using fragment object

        The values sent are whatever is in the text fields in stashapp
        (or otherwise internally current values)
        ...
        if fragment.get('code'):
            if '-' in fragment['code']:
                ...
                scene['code'] = fragment['code']

        # fragment.date
        scene['date'] = fragment.get('date')
        ...
        elif fragment.get('code'):
            scene['url'] = self.__get_movie_url_for_id(fragment['code'])
        ...
        if scene.get('url'):
            ...
            if len(movie_scenes) and scene_number:
        ...
        log.debug(f"_get_scene_by_fragment, scene: {scene}")
        return scene
