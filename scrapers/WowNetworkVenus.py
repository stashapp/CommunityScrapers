...
def search(title):
    ...
    for studio in MAIN_STUDIOS:
        ...
        if studio == 'Ultra Films':
            ...
            for studio_key in list(WOW_SUB_STUDIO_MAP.keys()):
                studio = WOW_SUB_STUDIO_MAP[studio_key]
...
def scene_card_parse(scene_card):
    title = scene_card.xpath(f'./a[@class="title"]/text()')[0].strip()
    imgurl = scene_card.xpath(f'.//img[@title]/@src')[0]
    imgurl= re.sub("_\w*", "_1280x720", imgurl)
    img = rGET(imgurl)
    b64img = base64.b64encode(img)
    performers = scene_card.xpath(f'.//*[@class="models"]/a/text()')
    tags = scene_card.xpath(f'.//span[@class="genres"]/a/text()')
...
def get_all_results(): #parse all scene elements, return all
    ...
    for studio, pages in search_results.items():
        query_studio = studio.replace(" ", "").lower()
        for page in pages:
            scene_cards = page.xpath(f'//div[contains(@class, "ct_video")]//img[@title]/ancestor::div')
            for scene_card in scene_cards:
                url = f'https://venus.{query_studio}.com' + scene_card.xpath(f'./a/@href')[0]
...
def get_scene_with_id(sceneID): #parse all scene elements, return single with matched id
    ...
    log.error('Scene not found!\nSome scenes doest not appear is search results unless you are logged in!')
    sys.exit()
