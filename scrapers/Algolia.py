...
def check_db(DB_PATH, SCENE_ID):
    try:
        ...
        log.debug("Database successfully connected to SQLite")
    except:
        log.warning("[Error] Fail to connect to the database")
...
def apikey_get(site_url,time):
    ...
    if script_html is not None:
        ...
        log.error("Can't retrieve API keys from the html ({})".format(site_url))
        return None, None
...
def check_config(domain, time):
    if os.path.isfile(STOCKAGE_FILE_APIKEY):
        ...
        try:
            time_past = datetime.datetime.strptime(config.get(domain, 'date'), '%Y-%m-%d %H:%M:%S.%f')
            ...
            if time_past.hour-1 < time.hour < time_past.hour+1 and (time - time_past).days == 0:
                ...
                log.info("Need new api key: [{}|{}|{}]".format(time.hour,time_past.hour,(time - time_past).days))
...
def json_parser(search_json, range_duration=60, single=False):
    ...
    for key,item in debug_dict.items():
        log.info("[{}] Title: {}; Ratio Title: {} - URL: {}".format(key,item["scene"],round(item["title"],3),round(item["url"],3)))
    log.info("--------------")
    #
...
def match_result(api_scene, range_duration=60, single=False):
    ...
    log.debug("[MATCH] Title: {} |-RATIO-| Title: {} / URL: {} |-MATCH-| Duration: {}, Size: {}, Domain: {}".format(api_title, round(match_ratio_title, 5), round(match_ratio_title_url, 5),match_duration, match_size, match_domain))
    match_dict = {}
...
def scraping_json(api_json, url=None):
    ...
    log.debug("[STUDIO] {} - {} - {} - {}".format(api_json.get('serie_name'),api_json.get('network_name'),api_json.get('mainChannelName'),api_json.get('sitename_pretty')))
    # Performer
    perf = []
    ...
    try:
        ...
        try:
            ...
            log.warning("Can't manage to get the image for some reason.")
    # URL
    try:
        scrape['url'] = 'https://{}.com/en/video/{}/{}/{}'.format(api_json["sitename"], api_json["sitename"], api_json["url_title"], api_json["clip_id"])
    except:
        if url:
            scrape['url'] = url
    #debug("{}".format(scrape))
...
HEADERS = {
    ...
    "Origin": "https://www.{}.com".format(SITE),
    "Referer": "https://www.{}.com".format(SITE)
}
...
if DB_PATH:
    if SCENE_ID:
        ...
        if database_dict is None:
            # Get data by SQlite
            log.warning("Fail to use GraphQL, trying to read database directly.")
        ...
        database_dict = None
        log.warning("Can't use the database because URL Scraping.")
else:
    database_dict = None
    log.warning("Can't find the database.")
...
if SEARCH_TITLE:
    ...
    if final_json is None:
        log.error("API Search don't give any result")
