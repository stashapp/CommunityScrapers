...
BASE_URL = "https://twinkloads.com"
SITEMAP_URL = "https://twinkloads.com/sitemap.xml"
    ...
    for url in urls:
        try:
            ...
            if page_resp.status_code != 200:
                continue
            match = re.search(r'content/(tld\d+)/', page_resp.text)
            if match:
...
def scrape_scene(url):
    """Scrape a scene from a TwinkLoads URL."""
    soup = fetch_page(url)
    if not soup:
        return {}
    ...
    if og_image and og_image.get("content"):
        code_match = re.search(r'content/(tld\d+)/', og_image["content"])
        if code_match:
            result["code"] = code_match.group(1)

    # Studio
    result["studio"] = {"name": "TwinkLoads"}
    ...
    for profile in soup.find_all("div", class_="modelProfile"):
        ...
        img = profile.find("img", attrs={"data-src": True})
        img_url = img["data-src"] if img else ""

        if name_tag:
            perf = {"name": name_tag, "gender": "Male"}
            if href:
                perf["url"] = href
            if img_url:
                perf["image"] = img_url
...
def scrape_performer(url):
    """Scrape a performer from a TwinkLoads model URL."""
    soup = fetch_page(url)
    if not soup:
        return {}
    ...
    # Image - check model profile sections for lazy-loaded images
    for section in soup.find_all("div", class_="modelProfile"):
        img = section.find("img", attrs={"data-src": True})
        if img and img["data-src"]:
            result["image"] = img["data-src"]
            result["images"] = [img["data-src"]]
            break
...
def extract_scene_code(text):
    """Extract a TwinkLoads scene code (e.g. tld0022) from a filename or string."""
    m = re.search(r'(?i)(?:^|[_\-\s])(tld\d{3,5})(?:$|[_\-\s.])', text)
    if m:
...
def scene_by_fragment():
    ...
    # If scene has a TwinkLoads URL, scrape it
    scene_url = fragment.get("url", "")
    if scene_url and "twinkloads.com/videos/" in scene_url:
    ...
    if not code:
        log.info(f"No TwinkLoads scene code found in '{source}'")
        print(json.dumps({}))
        return
...
def scene_by_name():
    ...
    try:
        ...
        urls = re.findall(r'<loc>(https://twinkloads\.com/videos/[^<]+)</loc>', resp.text)
